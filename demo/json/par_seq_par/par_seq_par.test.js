    ...
    it('Should call all parallel calls, and add api params at init and request time', function(done) {
      request(app)
        ...
        .end(function(err, res) {
          ...
          expect(res.text).toContain('"superNestedParallel1":{"statusCode":200,"systemMsg":"RESPONSE FROM /api/getdata/someothercall', 'res.text='+res.text);
          expect(res.text).toContain('"superNestedParallel2":{"statusCode":200,"systemMsg":"RESPONSE FROM /api/getdata/somecall', 'res.text='+res.text);
          done();
        });
    });
  });
